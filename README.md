...
* Step 1: Train network 1 for 100 epochs using loss function 1. Run the following: `python trainer_convnet.py --losstype 'anglesDC'`.
* Step 2: Compute a new dataset that has spatial map reconstructions from the PMR output of network 1. Run the following: `python validate_convnet1_savedepth.py`. 
* Step 3: Train network 2 for 100 epochs using loss function 2. Run the following: `python trainer_convnet.py --losstype 'anglesDC' --pmr`.
